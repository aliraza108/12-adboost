...
export function VariantCard({ variant }: { variant: Variant }) {
  ...
  return (
    <Card
      className={`relative p-5 ${variant.status === "winner" ? "border border-terracotta/60 shadow-[0_0_24px_rgba(200,96,42,0.25)]" : ""}`}
    >
      ...
      <p className="mt-2 text-sm text-sand-600">{variant.creative.body ?? "No body copy"}</p>
      <div className="mt-4">
      ...
      <div className="mt-4 flex flex-wrap gap-2">
        ...
        {variant.creative.emotional_trigger && (
          <Badge status="testing">{variant.creative.emotional_trigger}</Badge>
        )}
        <Badge status={variant.status === "winner" ? "winner" : "testing"}>{variant.status}</Badge>
      </div>
